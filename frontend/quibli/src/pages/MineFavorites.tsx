...
export default function MineFavorites() {
  ...
  const loadMorePosts = async () => {
    // 增加 cache.fetchingPostsPage 判断，防止同页重复请求
    if (loadingPosts || !cache.hasMorePosts || !user?.id || cache.fetchingPostsPage === cache.postsPage) return;
    
    setLoadingPosts(true);
    cache.fetchingPostsPage = cache.postsPage; // 加锁
    
    try {
      const res = await getFavoritePosts(user.id, cache.postsPage, 10);
      const newItems = res.postItems || [];
      const total = res.total || 0;

      // 去重逻辑：根据 id 过滤掉已经存在的项
  ...
  const loadMoreQuestions = async () => {
    ...
    try {
      const res = await getFavoriteQuestions(user.id, cache.questionsPage, 10);

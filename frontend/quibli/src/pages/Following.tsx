...
const cache = {
  ...
  fetchingPostsPage: 0,     // 请求锁
  fetchingQuestionsPage: 0, // 请求锁
  usersFetched: false,
};
...
export default function Following() {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-white flex flex-col overflow-hidden">
      ...
      {/* 用户头像列表 - 复用 User 类型 */}
      <div className="px-4 pt-3 pb-4 border-b border-gray-100 flex-shrink-0 bg-white">
        <div className="flex overflow-x-auto no-scrollbar space-x-5">
          {followedUsers.map(u => (
            <div key={u.id} onClick={() => handleUserClick(u.id)} className="flex flex-col items-center flex-shrink-0 w-16 cursor-pointer active:scale-95 transition-transform">
              <Avatar className="w-14 h-14 border-2 border-blue-100 p-0.5">
                <AvatarImage src={u.avatar} className="object-cover rounded-full" />
                <AvatarFallback className="bg-blue-50 text-blue-500 font-bold">{u.nickname?.[0]}</AvatarFallback>
              </Avatar>
              <span className="mt-2 text-xs text-gray-600 truncate w-full text-center">{u.nickname}</span>
            </div>
          ))}
      ...
      <div className="h-[48px] flex items-center justify-center relative bg-white border-b border-gray-100 flex-shrink-0">
        <button onClick={() => setActiveTab('posts')} className={`flex-1 h-full flex items-center justify-center text-[15px] font-medium transition-colors relative ${activeTab === 'posts' ? 'text-blue-600' : 'text-gray-500'}`}>
          文章{activeTab === 'posts' && <span className="absolute bottom-0 w-6 h-1 bg-blue-600 rounded-t-full" />}
        </button>
        ...
        <button onClick={() => setActiveTab('questions')} className={`flex-1 h-full flex items-center justify-center text-[15px] font-medium transition-colors relative ${activeTab === 'questions' ? 'text-blue-600' : 'text-gray-500'}`}>
          问题{activeTab === 'questions' && <span className="absolute bottom-0 w-6 h-1 bg-blue-600 rounded-t-full" />}
        </button>
      </div>

      <main className="flex-1 relative w-full h-full overflow-hidden bg-gray-50" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
              ...
              <InfiniteScroll onLoadMore={loadMorePosts} hasMore={cache.hasMorePosts} isLoading={loadingPosts}>
                <div className="pb-4">
                  ...
                  {!loadingPosts && posts.length === 0 && cache.usersFetched && (
                    <div className="py-20 text-center text-sm text-gray-400">暂无动态</div>
                  )}
                </div>
              </InfiniteScroll>
            </PullToRefresh>

...
export default function MineLikes() {
  ...
  const posts = useMineStore((s) => s.likesFeed.posts);
  const questions = useMineStore((s) => s.likesFeed.questions);
  const loadingPosts = useMineStore((s) => s.likesFeed.loadingPosts);
  const loadingQuestions = useMineStore((s) => s.likesFeed.loadingQuestions);
  const hasMorePosts = useMineStore((s) => s.likesFeed.hasMorePosts);
  const hasMoreQuestions = useMineStore((s) => s.likesFeed.hasMoreQuestions);
  const activeTab = useMineStore((s) => s.likesFeed.activeTab);
  const postsScrollY = useMineStore((s) => s.likesFeed.postsScrollY);
  const questionsScrollY = useMineStore((s) => s.likesFeed.questionsScrollY);

  const setActiveTab = useMineStore((s) => s.setLikesActiveTab);
  const setPostsScrollY = useMineStore((s) => s.setLikesPostsScrollY);
  const setQuestionsScrollY = useMineStore((s) => s.setLikesQuestionsScrollY);
  const loadMorePosts = useMineStore((s) => s.loadMoreLikesPosts);
  const loadMoreQuestions = useMineStore((s) => s.loadMoreLikesQuestions);
  const ensureInitialFetch = useMineStore((s) => s.ensureLikesInitialFetch);
